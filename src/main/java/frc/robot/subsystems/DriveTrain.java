  ...
  public void driveWithJoysticks(XboxController Controller, double speed) {
    if (Constants.DEADZONE * -1 < Controller.getRawAxis(Constants.XBOX_LEFT_Y_AXIS) * -1 && Controller.getRawAxis(Constants.XBOX_LEFT_Y_AXIS)* -1 < Constants.DEADZONE) {
      ...
      if (Controller.getRawAxis(Constants.XBOX_LEFT_Y_AXIS) * -1 > Constants.DEADZONE) {
        driveTrainSpeed = boost*(Constants.DRIVETRAINCURVE*(1-Constants.BASEVELOCITY)*Controller.getRawAxis(Constants.XBOX_LEFT_Y_AXIS)*-1+(1-Constants.DRIVETRAINCURVE)*(1-Constants.BASEVELOCITY)*Math.pow(Controller.getRawAxis(Constants.XBOX_LEFT_Y_AXIS)*-1, 5))+Constants.BASEVELOCITY;
      }
